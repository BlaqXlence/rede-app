...
export default function EventDetailScreen({ navigation, route }) {
    ...
    return (
      <SafeAreaView style={[s.safe, { backgroundColor: colors.background }]} edges={['top']}>
        <TouchableOpacity style={{ padding: 20 }} onPress={() => navigation.goBack()}>
          <Text style={[s.backTxtPlain, { color: colors.primary }]}>← Back</Text>
        </TouchableOpacity>
        <View style={s.center}>
  ...
  function handleJoin() {
    ...
    if (attending) {
      ...
      if (isPaid && hoursLeft > 0 && hoursLeft < 3) {
        Alert.alert(
          'Cannot leave paid event',
          `You can only leave paid events 3+ hours before they start. This event starts in ${Math.ceil(hoursLeft * 60)} minutes.`
        ); return
      }
  ...
  // DELETE — organiser can always delete
  function handleDelete() {
    if (!isOrganizer) return
    const hasAttendees = (event.attendeeCount || 0) > 0
    const msg = hasAttendees
      ? `"${event.title}" has ${event.attendeeCount} attendee(s). They will be notified it was cancelled. Delete anyway?`
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            Alert.alert('Could not delete', err.message || 'Check your connection.')
            setDeleting(false)
          }
        }
      },
...
const s = StyleSheet.create({
  ...
  backTxtPlain: { fontSize: 15, fontWeight: '600' },
